...
# guilds-param-analytics-channel-following-following-guild-size-get

## Goal

- Worker status: goal achieved.
- Objective: Implement production-ready GET support for `/guilds/{guild_id}/analytics/channel-following/following-guild-size` on the current integration branch, with focused tests, regenerated route catalogs and generated route artifacts, verification evidence, and a complete handoff report.
- Orchestrator acceptance: ported onto current base `91adbfc4e` and accepted after current-base regeneration.

## Summary

Implemented `GET /guilds/{guild_id}/analytics/channel-following/following-guild-size` for the assigned route only.
...
- Returns a conservative empty analytics array because Spacebar does not persist Discord's channel-following guild-size aggregate buckets yet.
- Exposes and validates `GuildChannelFollowingGuildSizeResponse` with documented guild-size bucket fields.

## Assigned Scope

- Assigned path: `/guilds/{param}/analytics/channel-following/following-guild-size`
- Owned method: `GET`
- Current-base missing count moved from 696 to 695; implemented route count moved from 484 to 485.
- Worker-base missing entries found before implementation: one
    - `GET /guilds/{param}/analytics/channel-following/following-guild-size`
    - route name `GET_GUILDS_GUILD_ID_ANALYTICS_CHANNEL_FOLLOWING_FOLLOWING_GUILD_SIZE`
...
- Confirmed absent before implementation:
    ...
    - No matching `src/api/routes/**` implementation for `channel-following/following-guild-size`.
...
- Userdoccers guild analytics docs: `https://raw.githubusercontent.com/discord-userdoccers/discord-userdoccers/master/pages/resources/guild-analytics.mdx`
    ...
    - Channel following guild-size fields: `day_pt`, `guild_size`, `total_guilds_following`.
...
- `src/schemas/responses/GuildChannelFollowingGuildSizeResponse.ts`
- `src/schemas/responses/index.ts`
- `test/routes/guilds-param-analytics-channel-following-following-guild-size-get.test.ts`
- `assets/schemas.json`
- `assets/openapi.json`
...
- `worker-progress/guilds-param-analytics-channel-following-following-guild-size-get.md`

## Current-Base Verification

- `npm run build:src:tsgo` passed.
- `npm run generate:schema` passed and wrote 955 schemas.
- `npm run build --workspace @spacebar/automatic-reverse-engineering` passed.
- `node packages/automatic-reverse-engineering/dist/cli.js import-source-routes --root src/api/routes --out packages/automatic-reverse-engineering/data/catalogs/routes.source.catalog.json` passed.
- `npm run build --workspace @spacebar/missing-routes` passed.
- `npm run start --workspace @spacebar/missing-routes` passed: `Spacebar is missing 695`, `Spacebar implements 485`, `Discord implements 1128`.
- `npm run generate:testing-manifest` passed and wrote 590 entries.
- `node scripts/testing-manifest/verify.js` passed.
- `npm run generate:contract-tests` passed and wrote 565 contracts.
- `node scripts/testing-manifest/generate-contract-tests.js --check` passed.
- `npm run generate:suite-coverage` passed and wrote 15 suites.
- `node scripts/testing-manifest/generate-suite-coverage.js --check` passed.
- `npm run generate:openapi` passed and wrote 391 paths and 955 schemas.
