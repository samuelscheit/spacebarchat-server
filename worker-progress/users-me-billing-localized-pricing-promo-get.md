...
- Original worker base: `Spacebar is missing 550`, `Spacebar implements 630`.
- Current pushed base before porting: `547` missing / `633` implemented.
- Current missing report after porting: `546` missing / `634` implemented /
  `1128` Discord.
...
- Ported over pushed base `e40cd14be`, preserving the saved-messages, Adyen,
  and invoice-breakdown route artifacts.
- Current-base regeneration wrote schemas/OpenAPI, source routes,
  missing-route, testing manifest, generated contract, and suite coverage
  artifacts. The current generated state has 739 manifest entries and 714 HTTP
  contracts.
- Current-base verification passed source build, test-fixture build, focused
  route tests `5/5`, manifest tests `30/30`, suite coverage tests `4/4`,
  generated contract matrix tests `10/10`, diff checks, and package/lockfile
  guard. The touched users supplemental scenario skipped because the Postgres
  admin URL is unavailable.
