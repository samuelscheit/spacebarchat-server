...
import type { PartialUser, PublicUser } from "@spacebar/schemas";
import { createApplicationCommandInteractionMessageData } from "../../src/api/util/utility/InteractionMetadata";
...
function makeCompletePublicUser(): PublicUser {
    return {
        id: "300",
        username: "alice",
        discriminator: "0001",
...
test("interaction callback stores projected interaction users", () => {
    ...
    assert.match(source, /user: interactionUser/);
});

test("interaction message data stores projected users on both interaction contracts", () => {
    const user = makeCompletePublicUser();
...
test("message history hydrates legacy interaction users through the shared partial-user projection", () => {
    ...
    assert.match(source, /hydrateInteractionMetadataUsers\(ret/);
