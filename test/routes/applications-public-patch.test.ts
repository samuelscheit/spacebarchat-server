...
import applicationsPublicRouter, {
    APPLICATIONS_PUBLIC_PATCH_UNSUPPORTED_MESSAGE,
    createApplicationsPublicPatchUnsupportedError,
} from "../../src/api/routes/applications/public";
...
describe("PATCH /applications/public", () => {
    ...
    test("generates source catalog, missing-route, OpenAPI, testing manifest, and contract metadata", () => {
        ...
        assert.equal(missingRoutes.missing_entries?.some((entry) => entry.method === "PATCH" && entry.route === "/applications/public"), false);
        assert.equal(missingRoutes.missing_entries?.some((entry) => entry.method === "GET" && entry.route === "/applications/public"), true);
        assert.equal(missingRoutes.missing_entries?.some((entry) => entry.method === "PUT" && entry.route === "/applications/public"), true);
