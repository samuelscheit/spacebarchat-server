...
import guildProductsRouter, {
    GUILD_PRODUCTS_CREATE_UNSUPPORTED_MESSAGE,
    createGuildProduct,
    createGuildProductsCreateUnsupportedError,
    createGuildProductsRouter,
    type GuildProductsRepositories,
} from "../../src/api/routes/guilds/#guild_id/products";

process.env.DATABASE ??= "postgres://spacebar:spacebar@localhost:5432/spacebar_route_test";

const requireModule = require;
const coveredManifestId = "api:http:POST:/guilds/:guild_id/products/";
const assignedPath = "/guilds/{param}/products";
const assignedRouteName = "GUILD_PRODUCTS";

describe("POST /guilds/:guild_id/products", () => {
    test("declares the assigned missing route context", () => {
        assert.equal(coveredManifestId, "api:http:POST:/guilds/:guild_id/products/");
        assert.equal(assignedPath, "/guilds/{param}/products");
        assert.equal(assignedRouteName, "GUILD_PRODUCTS");
    });

    test("stays behind bearer authentication", async () => {
        assert.equal(isNoAuthorizationRoute("POST", "/guilds/100000000000000001/products"), false);
        assert.equal(isNoAuthorizationRoute("POST", "/api/v9/guilds/100000000000000001/products"), false);

        const app = express();
        app.use(Authentication);
        app.use("/guilds/:guild_id/products", guildProductsRouter);
        app.use(ErrorHandler);

        const response = await requestJson(app, "/guilds/100000000000000001/products");
    ...
    test("requires MANAGE_GUILD, checks guild existence, and fails closed for authorized users", async (t) => {
        ...
        const response = await requestJson(createAuthenticatedRouteApp(repositories), "/guilds/100000000000000001/products");

        assert.equal(response.status, 501);
        assert.deepEqual(response.body, {
            code: 0,
            message: GUILD_PRODUCTS_CREATE_UNSUPPORTED_MESSAGE,
        });
    ...
    test("returns 403 when the authenticated user lacks MANAGE_GUILD", async (t) => {
        ...
        const response = await requestJson(
            ...
            "/guilds/100000000000000001/products",
        );
    ...
    test("uses existing not-found behavior when the guild does not exist", async (t) => {
        const findOptions = {
        ...
        await assert.rejects(() => createGuildProduct("missing-guild", { guildRepository: repository }), {
            name: "EntityNotFoundError",
        });
    });

    test("uses an explicit unsupported-provider API error", () => {
        const error = createGuildProductsCreateUnsupportedError();

        assert.equal(error.httpStatus, 501);
        assert.equal(error.code, 0);
        assert.equal(error.message, GUILD_PRODUCTS_CREATE_UNSUPPORTED_MESSAGE);
    });

    test("declares source-backed route metadata in source and generated artifacts", () => {
        const routeSource = readFileSync(join(process.cwd(), "src", "api", "routes", "guilds", "#guild_id", "products.ts"), "utf8");
        ...
        assert.match(routeSource, /summary:\s*"Create Guild Product"/);
        assert.match(routeSource, /provider-backed store, SKU, listing, attachment, entitlement, and payout state/);
        ...
        assert.doesNotMatch(routeSource, /router\.(get|patch|delete)\(/);
        assert.doesNotMatch(routeSource, /products\/attachments|products\/listings/);
        assert.doesNotMatch(routeSource, /200:\s*\{/);

        const route = openapi.paths?.["/guilds/{guild_id}/products/"]?.post;
        assert.equal(route?.summary, "Create Guild Product");
        ...
        assert.equal(manifestEntry?.sourceFile, "src/api/routes/guilds/#guild_id/products.ts");
        ...
        const catalogEntry = sourceCatalog.find((entry) => entry.method === "POST" && entry.route === "/guilds/{guild_id}/products");
        assert.equal(catalogEntry?.route_name, "POST_GUILDS_GUILD_ID_PRODUCTS");
        assert.equal(catalogEntry?.source, "src/api/routes/guilds/#guild_id/products.ts");
        ...
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "POST" && entry.route === "/guilds/{param}/products/attachments"),
            true,
        );
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.route === "/guilds/{param}/products/listings/{param}"),
            true,
        );
    });
});
...
function createAuthenticatedRouteApp(repositories: GuildProductsRepositories) {
    const app = express();
    ...
    app.use("/guilds/:guild_id/products", createGuildProductsRouter(repositories));
    app.use(ErrorHandler);
...
async function requestText(app: express.Express, requestPath: string): Promise<{ status: number; body: string }> {
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:${(address as AddressInfo).port}${requestPath}`, { method: "POST" });
