...
const coveredManifestId = "api:http:GET:/guilds/:guild_id/analytics/growth-activation/joins/";

function distModulePath(...segments: string[]) {
    return path.join(process.cwd(), "dist", ...segments);
}

const routeModulePath = distModulePath("api", "routes", "guilds", "#guild_id", "analytics", "growth-activation", "joins.js");

afterEach(() => {
    delete require.cache[routeModulePath];
});

describe("GET /guilds/:guild_id/analytics/growth-activation/joins", () => {
    test("registers the guild insights route metadata", (t) => {
        const harness = setupGuildGrowthActivationJoinsRoute(t);

        assert.deepEqual(harness.routeOptions, {
            summary: "Get Guild Growth Activation Joins",
            description: "Returns member join counts per aggregation interval.",
            ...
            responses: {
                200: {
                    body: "GuildGrowthActivationJoinsResponse",
                },
                401: {
    ...
    test("returns a conservative empty joins list for a valid analytics query", async (t) => {
        const harness = setupGuildGrowthActivationJoinsRoute(t);

        const response = await requestJson(
            harness.app,
            "/guilds/guild-id/analytics/growth-activation/joins?start=2026-05-01T00%3A00%3A00.000Z&end=2026-05-08T00%3A00%3A00.000Z&interval=1",
        );

        assert.equal(response.status, 200);
        assert.deepEqual(response.body, []);
        assert.deepEqual(harness.routeModule.createGuildGrowthActivationJoinsResponse(), []);
    ...
    test("rejects unsupported aggregation intervals before guild lookup", async (t) => {
        ...
        const response = await requestJson(harness.app, "/guilds/guild-id/analytics/growth-activation/joins?interval=4");
    ...
    test("rejects reversed analytics windows before guild lookup", async (t) => {
        ...
        const response = await requestJson(harness.app, "/guilds/guild-id/analytics/growth-activation/joins?start=2026-05-08T00%3A00%3A00.000Z&end=2026-05-01T00%3A00%3A00.000Z");
    ...
    test("validates documented joins buckets", () => {
        ...
        assert.equal(validator.ajv.validate("GuildGrowthActivationJoinsResponse", response), true);
        assert.equal(validator.ajv.validate("GuildGrowthActivationJoinsResponse", []), true);
        assert.equal(
            validator.ajv.validate("GuildGrowthActivationJoinsResponse", [
                {
                    day_pt: "2026-05-01T00:00:00Z",
                },
            ]),
    ...
    test("declares generated response schema, route catalog, contract, and manifest metadata", () => {
        ...
        assert.equal(schemas.GuildGrowthActivationJoinsResponse?.type, "array");
        assert.equal(schemas.GuildGrowthActivationJoinsResponse?.items?.$ref, "#/definitions/GuildGrowthActivationJoinsBucket");
        assert.deepEqual(schemas.GuildGrowthActivationJoinsBucket?.required?.sort(), ["day_pt", "joins"]);
        assert.equal(schemas.GuildGrowthActivationJoinsBucket?.properties?.day_pt?.type, "string");
        assert.equal(schemas.GuildGrowthActivationJoinsBucket?.properties?.joins?.type, "integer");
        assert.equal(schemas.GuildGrowthActivationJoinsBucket?.properties?.all_time_joins?.type, "integer");

        const route = openapi.paths?.["/guilds/{guild_id}/analytics/growth-activation/joins/"]?.get;
        assert.equal(route?.summary, "Get Guild Growth Activation Joins");
        assert.equal(route?.responses?.["200"]?.content?.["application/json"]?.schema?.$ref, "#/components/schemas/GuildGrowthActivationJoinsResponse");
        ...
        assert.equal(manifestEntry?.sourceFile, "src/api/routes/guilds/#guild_id/analytics/growth-activation/joins.ts");
        ...
        assert.equal(manifestEntry?.routeMetadata?.responseBodies?.includes("GuildGrowthActivationJoinsResponse"), true);
        ...
        const catalogEntry = sourceCatalog.find((entry) => entry.method === "GET" && entry.route === "/guilds/{guild_id}/analytics/growth-activation/joins");
        assert.equal(catalogEntry?.route_name, "GET_GUILDS_GUILD_ID_ANALYTICS_GROWTH_ACTIVATION_JOINS");
        assert.equal(catalogEntry?.source, "src/api/routes/guilds/#guild_id/analytics/growth-activation/joins.ts");
        assert.deepEqual(catalogEntry?.response_schema_refs?.sort(), ["APIErrorResponse", "GuildGrowthActivationJoinsResponse"]);

        const contract = contracts.contracts?.find((entry) => entry.manifestId === coveredManifestId);
        assert.equal(contract?.authMode, "bearer");
        assert.equal(contract?.path, "/guilds/:guild_id/analytics/growth-activation/joins/");
        assert.equal(contract?.routeMetadata?.permission, "VIEW_GUILD_INSIGHTS");
        assert.equal(contract?.routeMetadata?.responses?.includes("GuildGrowthActivationJoinsResponse"), true);
        ...
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "GET" && entry.route === "/guilds/{param}/analytics/growth-activation/joins"),
            false,
        );
    });
});

function setupGuildGrowthActivationJoinsRoute(t: TestContext) {
    ...
    delete require.cache[routeModulePath];
    const routeModule = requireModule(routeModulePath) as typeof import("../../src/api/routes/guilds/#guild_id/analytics/growth-activation/joins");
    const app = express();
    app.use("/guilds/:guild_id/analytics/growth-activation/joins", routeModule.default);
