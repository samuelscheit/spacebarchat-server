...
const soundId = "300000000000000003";
const viewerId = "100000000000000001";
const coveredManifestId = "api:http:GET:/guilds/:guild_id/soundboard-sounds/:sound_id/";

describe("GET /guilds/:guild_id/soundboard-sounds/:sound_id", () => {
    test("stays behind bearer authentication", async (t) => {
        const dependencies = createThrowingDependencies(t);

        assert.equal(isNoAuthorizationRoute("GET", `/api/v9/guilds/${guildId}/soundboard-sounds/${soundId}`), false);

        const response = await requestJson(createAuthenticatedApp(dependencies), `/guilds/${guildId}/soundboard-sounds/${soundId}`);
        ...
        assert.equal(mockOf(dependencies.findGuildSoundboardSound!).mock.callCount(), 0);
    });

    test("returns a source-backed guild soundboard sound and strips creator details without expression permissions", async (t) => {
        const sound = createSoundboardSound();
        const dependencies = createDependencies(t, {
            findGuildSoundboardSound: t.mock.fn(async () => sound),
        });

        const response = await requestJson(createApp(dependencies), `/guilds/${guildId}/soundboard-sounds/${soundId}`);
        ...
        assert.deepEqual(mockOf(dependencies.findGuildSoundboardSound!).mock.calls[0].arguments, [guildId, soundId, { includeUser: false }]);
        assert.equal(mockOf(dependencies.findGuildSoundboardSounds).mock.callCount(), 0);
    });
    ...
    test("includes creator details when the requester can manage or create expressions", async (t) => {
        const dependencies = createDependencies(t, {
            ...
            findGuildSoundboardSound: t.mock.fn(async () => createSoundboardSound()),
        });

        const response = await getGuildSoundboardSoundResponse(guildId, soundId, viewerId, dependencies);

        assert.equal(response.user?.id, "400000000000000004");
        assert.deepEqual(mockOf(dependencies.findGuildSoundboardSound!).mock.calls[0].arguments, [guildId, soundId, { includeUser: true }]);
    });

    test("can reuse the guild soundboard list dependency when no direct lookup is supplied", async (t) => {
        const dependencies = createDependencies(t, {
            findGuildSoundboardSound: undefined,
            findGuildSoundboardSounds: t.mock.fn(async () => [createSoundboardSound({ sound_id: "300000000000000004" }), createSoundboardSound()]),
        });

        const response = await getGuildSoundboardSoundResponse(guildId, soundId, viewerId, dependencies);

        assert.equal(response.sound_id, soundId);
    ...
    test("returns unknown sound when the guild member has no backed soundboard match", async (t) => {
        const dependencies = createDependencies(t, {
            findGuildSoundboardSound: t.mock.fn(async () => null),
        });
    ...
    test("declares response schemas and generated route artifacts", () => {
        ...
        assert.match(routeSource, /summary:\s*"Get Guild Soundboard Sound"/);
        assert.match(routeSource, /200:\s*\{\s*body:\s*"SoundboardSoundResponse"/s);
        assert.match(routeSource, /401:\s*\{\s*body:\s*"APIErrorResponse"/s);
        assert.match(routeSource, /403:\s*\{\s*body:\s*"APIErrorResponse"/s);
        assert.match(routeSource, /404:\s*\{\s*body:\s*"APIErrorResponse"/s);

        assert.equal(schemas.SoundboardSoundResponse?.type, "object");
        ...
        const route = openapi.paths?.["/guilds/{guild_id}/soundboard-sounds/{sound_id}/"]?.get ?? openapi.paths?.["/guilds/{guild_id}/soundboard-sounds/{sound_id}"]?.get;
        assert.equal(route?.summary, "Get Guild Soundboard Sound");
        assert.equal(route?.responses?.["200"]?.content?.["application/json"]?.schema?.$ref, "#/components/schemas/SoundboardSoundResponse");
        ...
        assert.equal(manifestEntry?.routeMetadata?.responseBodies?.includes("SoundboardSoundResponse"), true);
        ...
        const catalogEntry = sourceCatalog.find((entry) => entry.method === "GET" && entry.route === "/guilds/{guild_id}/soundboard-sounds/{sound_id}");
        assert.equal(catalogEntry?.route_name, "GET_GUILDS_GUILD_ID_SOUNDBOARD_SOUNDS_SOUND_ID");
        assert.equal(catalogEntry?.source, "src/api/routes/guilds/#guild_id/soundboard-sounds.ts");
        assert.deepEqual(catalogEntry?.response_schema_refs?.sort(), ["APIErrorResponse", "SoundboardSoundResponse"]);

        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "GET" && entry.route === "/guilds/{param}/soundboard-sounds/{param}"),
            false,
        );
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "DELETE" && entry.route === "/guilds/{param}/soundboard-sounds/{param}"),
            true,
        );
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "PATCH" && entry.route === "/guilds/{param}/soundboard-sounds/{param}"),
            true,
        );
    });
});
...
function createThrowingDependencies(t: TestContext) {
    return createDependencies(t, {
        ...
        findGuildSoundboardSounds: t.mock.fn(async () => {
            throw new Error("sound list lookup should not run");
        }),
        findGuildSoundboardSound: t.mock.fn(async () => {
...
function createSoundboardSound(overrides: Partial<SoundboardSoundResponse> = {}): SoundboardSoundResponse {
    return {
        sound_id: soundId,
        name: "Air horn",
        volume: 0.8,
