...
describe("GET /guilds/:guild_id/requests", () => {
    ...
    test("declares source-backed route metadata in source and generated artifacts", () => {
        ...
        assert.match(routeSource, /summary:\s*"Get Guild Join Requests"/);
        assert.match(routeSource, /does not currently persist Discord's guild join request queue/);
        assert.match(routeSource, /permission:\s*"MANAGE_GUILD"/);
        assert.doesNotMatch(routeSource, /router\.(?:post|put|patch|delete)\(/);
        assert.doesNotMatch(routeSource, /requests\/@me|join-requests|member-verification|new-member-action|onboarding/);
        assert.match(routeSource, /200:\s*\{\s*body:\s*"GuildJoinRequestsResponse"/s);
        ...
        assert.equal(
            sourceCatalog.some((entry) => entry.route === "/guilds/{guild_id}/requests/@me"),
            false,
        );
        ...
        assert.equal(hasMissingRoute(missingRoutes, "GET", "/guilds/{param}/requests/@me"), true);
