...
test(
    ...
    async () => {
        ...
        try {
            ...
            gateway = await startGateway();
            ownerClient = await connectIdentifiedGatewayClient(gateway.url, ownerToken, { intents: streamGatewayIntents });
            ...
            viewerClient = await connectIdentifiedGatewayClient(gateway.url, viewerToken, { intents: streamGatewayIntents });
...
async function connectIdentifiedGatewayClient(gatewayUrl: string, token: string, options: { intents?: number } = {}) {
