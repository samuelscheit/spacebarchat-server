...
import {
    ...
    WIDGET_STYLES,
    WIDGET_STYLE_ERROR,
} from "../../src/api/util/GuildWidgetPngRenderer";

const expectedDimensions: Record<WidgetImageStyle, { width: number; height: number }> = {
...
test("widget PNG renderer supports every declared style without requiring a guild icon", async () => {
    ...
    for (const style of WIDGET_STYLES) {
        assert.equal(isWidgetImageStyle(style), true);
        const png = await renderGuildWidgetPng({ style, name: "No Icon Guild", presence: "0 ONLINE" });
        assertPngDimensions(png, expectedDimensions[style]);
    }

    assert.equal(isWidgetImageStyle("invalid"), false);
});
...
test("widget PNG renderer composites an optional icon when one is available", async () => {
    ...
    const withoutIcon = await renderGuildWidgetPng({ style: "banner1", name: "Icon Guild", presence: "7 ONLINE" });
    const withIcon = await renderGuildWidgetPng({ style: "banner1", name: "Icon Guild", presence: "7 ONLINE", iconDataUri: imageBufferToDataUri(icon) });
