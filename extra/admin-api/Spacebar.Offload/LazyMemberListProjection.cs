...
public static class LazyMemberListProjection {
    ...
    public static GuildMemberListUpdate BuildUpdate(
        ...
        bool includePresences = true
    ) {
        var snapshot = BuildSnapshot(guildId, members, includePresences);
    ...
    private static LazyMemberListSnapshot BuildSnapshot(long guildId, IReadOnlyCollection<DbMember> members, bool includePresences) {
        ...
        var membersWithPresence = members
            .Select(member => new MemberListEntry(
                ...
                member,
                includePresences ? GetPublicSession(member) : null
            ))
        ...
        foreach (var roleId in displayRoles) {
            ...
            items.AddRange(roleMembers.Select(entry => ToSyncItem(entry.Member, entry.Session, includePresences)));
        }
        ...
        if (offlineMembers.Count > 0) {
            ...
            groups.Add(group);
            items.Add(new GuildMemberListSyncItem { Group = group });
            items.AddRange(offlineMembers.Select(entry => ToSyncItem(entry.Member, entry.Session, includePresences)));
        }
    ...
    private static GuildMemberListSyncItem ToSyncItem(DbMember member, DbSession? session, bool includePresence) {
        var publicMember = member.ToPublicMember();
        var presence = includePresence ? BuildMemberListPresence(member, session) : null;
