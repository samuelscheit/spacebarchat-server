    ...
    private async Task<GuildSyncResponse> GetGuildSyncAsync(long guildId)
    {
        ...
        var members = await _db.Members.AsNoTracking().Where(x => x.GuildId == guildId)
            ...
            .ThenInclude(x => x.Sessions.Where(s =>
                !s.IsAdminSession && !SessionPresenceProjection.NonVisibleStatuses.Contains(s.Status) && (!isLargeGuild || s.LastSeen >= offlineTreshold)))
        ...
        var presences = members.Select(x => x.IdNavigation).Where(x => x.Sessions.Count > 0).ToFrozenDictionary(x => x.Id, x =>
        {
            ...
            return new Presence()
            {
                ...
                Activities = x.Sessions.Where(SessionPresenceProjection.IsVisiblePresence)
