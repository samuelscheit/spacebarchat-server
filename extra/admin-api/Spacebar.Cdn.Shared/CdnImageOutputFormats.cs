...
public static class CdnImageOutputFormats {
    ...
    private static readonly Dictionary<string, string> MimeTypesByFormatName = SafeFormatsByExtension.Values
        .DistinctBy(format => format.MagickFormatName, StringComparer.OrdinalIgnoreCase)
        .ToDictionary(format => format.MagickFormatName, format => format.MimeType, StringComparer.OrdinalIgnoreCase);
