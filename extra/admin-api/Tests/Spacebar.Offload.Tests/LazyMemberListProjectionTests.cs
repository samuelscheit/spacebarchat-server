...
public class LazyMemberListProjectionTests {
    ...
    [Fact]
    public void BuildUpdateOmitsPresenceDataWhenPresenceIntentIsAbsent() {
        var guildRole = Role(10, 10, 0, hoist: false);
        var update = LazyMemberListProjection.BuildUpdate(10, "everyone", [
            Member(1, "Ada", [guildRole], [Session("online")]),
        ], [[0, 1]], includePresences: false);

        Assert.Equal(0, update.OnlineCount);
        Assert.Equal(["offline"], update.Groups.Select(group => group.Id).ToArray());

        var op = Assert.IsType<GuildMemberListSyncOperation>(Assert.Single(update.Operations));
        Assert.Null(op.Items[1].Member?.Presence);
        ...
        Assert.DoesNotContain("\"presence\"", json, StringComparison.Ordinal);
    }
