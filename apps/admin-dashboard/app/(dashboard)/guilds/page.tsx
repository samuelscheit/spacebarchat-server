...
export default async function GuildsPage({ searchParams }: { searchParams: Promise<{ q?: string; offset?: string }> }) {
    const params = await searchParams;
    const offset = parseOffsetParam(params.offset);
    const guilds = await safeAdminFetch<PageResult<AdminGuildListItem>>(`/guilds${queryString({ q: params.q, limit: 50, offset })}`);
