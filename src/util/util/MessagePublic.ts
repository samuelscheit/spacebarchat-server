...
interface PublicMessageSource {
    ...
    mention_roles?: { id: string }[];
    mentions?: (PublicUserSource | PartialUser | null | undefined)[];
...
function toPartialUser(user: PublicUserSource | PartialUser | null | undefined): PartialUser {
    if (user && "toPublicUser" in user && !user.toPublicUser) console.trace("toPublic user missing!!!");
    return (user && "toPublicUser" in user && user.toPublicUser ? user.toPublicUser() : (user ?? undefined)) as unknown as PartialUser;
}
...
export function messageToPublicMessage(message: PublicMessageSource, shallow = false): PublicMessage {
    ...
    return {
        ...
        mentions: message.mentions?.map(toPartialUser) ?? [],
