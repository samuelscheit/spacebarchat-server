...
describe("Message mention serialization", () => {
    ...
    test("Message.toJSON exposes channel mentions as partial channels", () => {
        ...
        const json = message.toJSON();
        const channelMention = json.mention_channels?.[0] as unknown as Record<string, unknown>;

        assert.deepEqual(json.mention_channels, [serializedChannelMention]);
        assert.equal(channelMention.topic, undefined);
        assert.equal(channelMention.permission_overwrites, undefined);
        assert.equal(channelMention.last_message_id, undefined);
    });
    ...
    test("search hits reuse public message mention serialization", () => {
        ...
        assert.equal((hit.mention_channels?.[0] as unknown as Record<string, unknown>).topic, undefined);
    });
