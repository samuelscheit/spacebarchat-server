...
export const applyReadyChannelOrdering = <T extends ReadyChannel>(channels: T[], channelOrdering: readonly string[] | null | undefined): (T & { position: number })[] => {
    ...
    const groupReadyRank = ({ channel, rank }: IndexedChannel<T>) => Math.min(rank, ...(childrenByParentId.get(channel.id) ?? []).map((child) => child.rank));
    const byReadyGroupRank = (a: IndexedChannel<T>, b: IndexedChannel<T>) => groupReadyRank(a) - groupReadyRank(b) || byReadyRank(a, b);

    topLevelChannels.sort(byReadyGroupRank).forEach(({ channel }, position) => {
