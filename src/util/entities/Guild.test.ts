...
describe("Guild.createGuild", () => {
    test("persists imported template channel ordering from serialized channel order", async () => {
        ...
        const channelCreateCalls: { channel: { id?: string; name?: string; parent_id?: string }; options?: { skipOrdering?: boolean } }[] = [];
        const guildUpdates: { criteria: unknown; partial: { channel_ordering?: string[] } }[] = [];
        ...
        try {
            ...
            const guild = await Guild.createGuild({
                ...
                channels: [
                    ...
                    { id: "child-1", name: "child-1", type: 0, parent_id: "category", position: 99 },
            ...
            assert.deepEqual(guildUpdates.at(-1), {
                ...
                partial: { channel_ordering: ["new-category", "new-child-1", "new-child-2", "new-top-level"] },
            });
        } finally {
