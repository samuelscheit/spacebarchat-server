...
import { buildReadySupplementalData, buildReadySupplementalGuilds } from "./ReadySupplemental";
import type { PublicVoiceState } from "@spacebar/schemas";
import type { Activity, GuildOrUnavailable, Session, VoiceState } from "@spacebar/util";

function guild(id: string, voiceStates: unknown[] = []): GuildOrUnavailable {
    return {
        id,
...
describe("READY_SUPPLEMENTAL payload", () => {
    ...
    test("serializes persisted voice states without leaking internal fields", () => {
        const payload = buildReadySupplementalData([
            guild("guild-with-voice", [
                {
        ...
        assert.equal("token" in payload.guilds[0].voice_states[0], false);
    });

    test("accepts already public voice-state payloads", () => {
        const publicVoiceState: PublicVoiceState = {
            channel_id: "voice-channel",
            deaf: false,
            guild_id: "guild-with-public-voice",
            mute: false,
        ...
        assert.deepEqual(buildReadySupplementalGuilds([guild("guild-with-public-voice", [publicVoiceState])])[0].voice_states, [publicVoiceState]);
    });
});
