...
function installHarness(options: { botsCanUseInvites?: boolean; memberCount?: number; guildFeatures?: string[] } = {}) {
    ...
    const dependencies = {
        configGet: () => ({ user: { botsCanUseInvites: options.botsCanUseInvites ?? false } }),
...
describe("joinGuildMember", () => {
    ...
    test("preserves the bot invite gate for self joins", async () => {
        const harness = installHarness();
        await assert.rejects(
            joinGuildMember({ guild_id: "guild-id", member_id: "@me", user_id: "bot-id", user_bot: true }, harness.dependencies),
            DiscordApiErrors.BOT_PROHIBITED_ENDPOINT,
        );
        assert.deepEqual(harness.findCalls, []);
        assert.deepEqual(harness.addToGuildCalls, []);
    });

    test("preserves the discoverable guild requirement", async () => {
        const harness = installHarness({ guildFeatures: [] });
        await assert.rejects(joinGuildMember({ guild_id: "guild-id", member_id: "@me", user_id: "user-id" }, harness.dependencies), DiscordApiErrors.UNKNOWN_GUILD);
        assert.deepEqual(
            harness.findCalls.map((x) => x.entity),
            ["Guild.findOneOrFail"],
        );
