...
describe("handleMessage", () => {
    ...
    test("rejects a new user message inside channel slowmode", async (t) => {
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof ApiError && error.code === 20016,
        );
    ...
    test("allows channel slowmode bypass permissions", async (t) => {
        ...
        const permission = {
            cache: {},
            has: (permissionName: string) => permissionName === "BYPASS_SLOWMODE",
            hasThrow: () => undefined,
        };
        const rights = {
            hasThrow: () => undefined,
        ...
        assert.equal(createMessageMock.mock.callCount(), 1);
    });
