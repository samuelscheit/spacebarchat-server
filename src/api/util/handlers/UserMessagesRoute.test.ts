...
describe("user direct-message route", () => {
    test("delegates POST sends to the shared channel message creation handlers", () => {
        ...
        assert.match(source, /import \{ createMessageBodyRouteHandlers, createMessageResolvedChannelRouteHandlers \} from "\.\.\/\.\.\/channels\/#channel_id\/messages\/index";/);
    ...
    test("channel message route keeps canonical POST handlers exported and mounted", () => {
        ...
        assert.match(source, /export const loadMessageChannelPermissions: RequestHandler = async/);
        ...
        assert.match(source, /export const createMessageResolvedChannelRouteHandlers: RequestHandler\[\] = \[loadMessageChannelPermissions, createMessageHandler\];/);
