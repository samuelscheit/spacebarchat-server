...
import { Config, DiscordApiErrors, Emoji, getRights, Guild, GuildFeature, Member, Role, Sticker } from "@spacebar/util";
...
export async function joinGuildMember(input: GuildMemberJoinInput, dependencies: GuildMemberJoinDependencies = defaultDependencies): Promise<GuildMemberJoinResult> {
    ...
    if (!guild.features.includes(GuildFeature.Discoverable)) throw DiscordApiErrors.UNKNOWN_GUILD;
