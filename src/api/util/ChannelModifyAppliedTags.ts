...
export function addInvalidAppliedTagsError(payload: Pick<ChannelModifySchema, "applied_tags">, isThread: boolean, errors: ErrorList) {
    if (payload.applied_tags === undefined || isThread) return;

    errors["applied_tags"] = makeObjectErrorContent("BASE_TYPE_BAD_VALUE", "Applied tags can only be set on threads");
}
