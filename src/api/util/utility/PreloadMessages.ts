...
export type PreloadMessagesOptions<T extends PreloadableMessage, TResponse = PreloadMessageJson> = {
    getAuthorizedChannelIds(channelIds: string[]): Promise<Set<string>>;
    findLatestMessage(channelId: string): Promise<T | null>;
    serializeMessage?: (message: T) => TResponse;
};
...
export async function preloadAuthorizedMessages<T extends PreloadableMessage, TResponse = PreloadMessageJson>(
    channelIds: string[],
    { getAuthorizedChannelIds, findLatestMessage, serializeMessage = serializePreloadedMessage as (message: T) => TResponse }: PreloadMessagesOptions<T, TResponse>,
) {
