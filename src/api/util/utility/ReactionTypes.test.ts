...
describe("parseReactionTypeParam", () => {
    ...
    it("centralizes message reaction serialization through the public reaction serializer", () => {
        const messageEntityPath = path.resolve(process.cwd(), "src/util/entities/Message.ts");
        const source = fs.readFileSync(messageEntityPath, "utf8");

        assert.match(source, /reactions:\s*this\.reactions\s*\?\s*toPublicReactions\(this\.reactions\)\s*:\s*undefined/);
