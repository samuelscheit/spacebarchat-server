...
const { getWidgetJsonData, getWidgetMemberSample, getWidgetOnlineMemberCount, toWidgetMember } = require("./widget.json.js") as {
    ...
    toWidgetMember(guildId: string, member: MockMember, now?: number): { avatar_url: string; id: string; status: string; username: string } | undefined;
};
...
test("getWidgetMemberSample queries a random capped sample of distinct recently visible members before hydration", async () => {
    ...
    assert.deepEqual(queryCalls[2].args, [
        "user.sessions",
        "session",
        "session.last_seen > :minLastSeen AND session.is_admin_session = false",
        { minLastSeen: new Date("2026-05-08T11:55:00Z") },
    ]);
...
test("getWidgetJsonData uses the uncapped online member count for presence_count", async (t) => {
    ...
    const member: MockMember = {
        ...
        user: {
            ...
            sessions: [
                ...
                { status: "online", last_seen: new Date(fixedNow), getPublicStatus: () => "online", is_admin_session: true },
...
test("toWidgetMember skips members without a schema-visible recent status", () => {
    const member: MockMember = {
        id: "44",
        user: {
            discriminator: "0001",
    ...
    assert.equal(toWidgetMember("guild", member, fixedNow), undefined);
});
