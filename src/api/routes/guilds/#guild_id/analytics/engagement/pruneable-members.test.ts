...
const coveredManifestId = "api:http:GET:/guilds/:guild_id/analytics/engagement/pruneable-members/";

function distModulePath(...segments: string[]) {
    return path.join(process.cwd(), "dist", ...segments);
}

const routeModulePath = distModulePath("api", "routes", "guilds", "#guild_id", "analytics", "engagement", "pruneable-members.js");

afterEach(() => {
    delete require.cache[routeModulePath];
});

describe("GET /guilds/:guild_id/analytics/engagement/pruneable-members", () => {
    test("registers the guild insights route metadata", (t) => {
        const harness = setupGuildEngagementPruneableMembersRoute(t);

        assert.deepEqual(harness.routeOptions, {
            summary: "Get Guild Engagement Pruneable Members",
            description: "Returns inactive member analytics buckets for the requested aggregation interval.",
            ...
            responses: {
                200: {
                    body: "GuildEngagementPruneableMembersResponse",
                },
                401: {
    ...
    test("returns a conservative empty pruneable-member engagement list for a valid analytics query", async (t) => {
        const harness = setupGuildEngagementPruneableMembersRoute(t);

        const response = await requestJson(
            harness.app,
            "/guilds/guild-id/analytics/engagement/pruneable-members?start=2026-01-01T00%3A00%3A00.000Z&end=2026-01-02T00%3A00%3A00.000Z&interval=1",
        );

        assert.equal(response.status, 200);
        assert.deepEqual(response.body, []);
        assert.deepEqual(harness.routeModule.getGuildEngagementPruneableMembers(), []);
    ...
    test("rejects unsupported aggregation intervals before guild lookup", async (t) => {
        ...
        const response = await requestJson(harness.app, "/guilds/guild-id/analytics/engagement/pruneable-members?interval=4");
    ...
    test("declares generated response schema, route catalog, manifest, and contract metadata", () => {
        ...
        assert.equal(schemas.GuildEngagementPruneableMembersResponse?.type, "array");
        assert.equal(schemas.GuildEngagementPruneableMembersResponse?.items?.$ref, "#/definitions/GuildEngagementPruneableMembersBucket");
        assert.deepEqual(schemas.GuildEngagementPruneableMembersBucket?.required?.sort(), ["day_pt", "inactive"]);
        assert.equal(schemas.GuildEngagementPruneableMembersBucket?.properties?.day_pt?.type, "string");
        assert.equal(schemas.GuildEngagementPruneableMembersBucket?.properties?.inactive?.type, "integer");

        const route = openapi.paths?.["/guilds/{guild_id}/analytics/engagement/pruneable-members/"]?.get;
        assert.equal(route?.summary, "Get Guild Engagement Pruneable Members");
        assert.equal(route?.responses?.["200"]?.content?.["application/json"]?.schema?.$ref, "#/components/schemas/GuildEngagementPruneableMembersResponse");
        ...
        assert.equal(manifestEntry?.sourceFile, "src/api/routes/guilds/#guild_id/analytics/engagement/pruneable-members.ts");
        ...
        assert.equal(manifestEntry?.routeMetadata?.responseBodies?.includes("GuildEngagementPruneableMembersResponse"), true);
        ...
        const catalogEntry = sourceCatalog.find((entry) => entry.method === "GET" && entry.route === "/guilds/{guild_id}/analytics/engagement/pruneable-members");
        assert.equal(catalogEntry?.route_name, "GET_GUILDS_GUILD_ID_ANALYTICS_ENGAGEMENT_PRUNEABLE_MEMBERS");
        assert.equal(catalogEntry?.source, "src/api/routes/guilds/#guild_id/analytics/engagement/pruneable-members.ts");
        assert.deepEqual(catalogEntry?.response_schema_refs?.sort(), ["APIErrorResponse", "GuildEngagementPruneableMembersResponse"]);

        const contract = contracts.contracts?.find((entry) => entry.manifestId === coveredManifestId);
        assert.equal(contract?.authMode, "bearer");
        assert.equal(contract?.path, "/guilds/:guild_id/analytics/engagement/pruneable-members/");
        assert.equal(contract?.routeMetadata?.permission, "VIEW_GUILD_INSIGHTS");
        assert.equal(contract?.routeMetadata?.responses?.includes("GuildEngagementPruneableMembersResponse"), true);
        ...
        assert.equal(
            missingRoutes.missing_entries.some((entry) => entry.method === "GET" && entry.route === "/guilds/{param}/analytics/engagement/pruneable-members"),
            false,
        );
    });
});

function setupGuildEngagementPruneableMembersRoute(t: TestContext) {
    ...
    delete require.cache[routeModulePath];
    const routeModule = requireModule(routeModulePath) as typeof import("./pruneable-members");
    const app = express();
    app.use("/guilds/:guild_id/analytics/engagement/pruneable-members", routeModule.default);
