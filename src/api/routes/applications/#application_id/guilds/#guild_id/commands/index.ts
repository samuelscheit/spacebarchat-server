...
import { Application, ApplicationCommand, Guild, Member, Snowflake } from "@spacebar/util";
...
router.get("/", route({}), async (req: Request, res: Response) => {
    ...
    const command = await ApplicationCommand.find({
        where: applicationCommandScopeWhere({ applicationId: req.params.application_id as string, guildId: req.params.guild_id as string }),
    });
    res.send(command);
});
